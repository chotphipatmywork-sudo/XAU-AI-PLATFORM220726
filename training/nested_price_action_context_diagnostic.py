"""Nested confirmation of one controlled-promoted CR-006 feature set."""
...
def authorized_feature_sets(controlled_report: dict[str, Any]) -> tuple[str, str]:
    if controlled_report.get("diagnostic_stage") != (
        "train_only_past_price_action_context_comparison"
    ):
        raise ValueError("Unexpected controlled CR-006 report")
    if controlled_report.get("validation_dataset_used") or controlled_report.get(
        "test_dataset_used"
    ):
        raise ValueError("Controlled CR-006 report is not Train-only")
    if controlled_report.get("source_feature_schema_version") != FEATURE_SCHEMA_VERSION:
        raise ValueError("Controlled CR-006 Feature Schema does not match")
    if controlled_report.get("label_schema_version") != LABEL_SCHEMA_VERSION:
        raise ValueError("Controlled CR-006 Label Schema does not match")
    if not controlled_report.get("nested_confirmation_authorized"):
        raise ValueError("Controlled CR-006 promotion did not authorize nested confirmation")
    ...
    if promoted not in registered or promoted == BASELINE:
        raise ValueError("Controlled CR-006 report has no valid promoted feature set")
...
def main() -> None:
    ...
    if arguments.outer_folds != 4 or arguments.inner_folds != 3:
        raise ValueError("CR-006 nested confirmation requires 4 Outer and 3 Inner folds")
    ...
    report = {
        "diagnostic_stage": "train_only_nested_price_action_context_confirmation",
        ...
        "price_action_context_coverage": context_coverage(context),
        **nested,
