"""Compare bounded past-only price-action context on purged Train folds."""
...
CONTEXT_FEATURE_COLUMNS = CONTEXT_COLUMNS[2:10]
...
def read_price_action_context(path: Path) -> dict[tuple[int, str], list[float]]:
    context: dict[tuple[int, str], list[float]] = {}
    with path.open("r", newline="", encoding="utf-8") as source:
        reader = csv.DictReader(source)
        if reader.fieldnames is None or tuple(reader.fieldnames) != CONTEXT_COLUMNS:
            raise ValueError(f"Unexpected Price Action CSV schema in {path}")
        for row_number, row in enumerate(reader, start=2):
            try:
                key = (int(row["id"]), str(row["timestamp"]))
                values = [float(row[name]) for name in CONTEXT_FEATURE_COLUMNS]
                valid = float(row["price_action_valid"])
            except (KeyError, TypeError, ValueError) as error:
                raise ValueError(
                    f"Invalid Price Action context at row {row_number}"
                ) from error
            if key in context:
                raise ValueError(f"Duplicate Price Action key at row {row_number}: {key}")
            if any(value < 0.0 or value > 100.0 for value in values):
                raise ValueError(f"Out-of-range Price Action value at row {row_number}")
            if valid not in (0.0, 100.0):
                raise ValueError(f"Invalid Price Action validity at row {row_number}")
            if valid == 0.0 and any(value != 50.0 for value in values):
                raise ValueError(
                    f"Invalid Price Action row is not neutral at row {row_number}"
                )
            context[key] = values + [valid]
    if not context:
        raise ValueError(f"Price Action research dataset is empty: {path}")
    return context
...
def context_coverage(
    ...
    valid = sum(values[8] == 100.0 for values in context.values())
    total = len(context)
    return {
        "total_records": total,
        "valid_records": valid,
...
def append_context(
    ...
    if any(index < 0 or index >= len(CONTEXT_FEATURE_COLUMNS) for index in indices):
        raise ValueError("Price Action context index is outside the registered fields")
    missing = [key for key in keys if key not in context]
    if missing:
        raise ValueError(f"Missing Price Action context for {len(missing)} rows")
    return [
...
def feature_set_specs() -> list[dict[str, Any]]:
    return [
        ...
        {"name": "direct_price_momentum", "context_indices": [0, 1, 2]},
        {"name": "completed_candle_impulse", "context_indices": [3, 4, 5]},
        {"name": "prior_range_context", "context_indices": [6, 7]},
        {"name": "all_price_action_context", "context_indices": list(range(8))},
    ]
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--price-action-context", required=True, type=Path)
...
def main() -> None:
    ...
    if arguments.folds != 4:
        raise ValueError("CR-006 controlled comparison requires exactly four folds")
    features, labels = read_dataset(arguments.train)
    keys = read_dataset_keys(arguments.train)
    context = read_price_action_context(arguments.price_action_context)
    feature_sets = {
    ...
    report = {
        ...
        **contract_metadata(),
        ...
        "price_action_context_records": len(context),
        "price_action_context_coverage": coverage,
        ...
        "limitations": [
            "Only Train and the auxiliary completed-bar research file are read.",
