...
$logRoot = Join-Path $projectRoot "outputs\compile\session_parity"
$targets = @(
    "tests\TestHistoricalRuntimeSessionParity.mq5",
    "tests\TestSessionFeatureProgress.mq5",
    "tests\TestHistoricalBrainReplay.mq5",
    "tests\TestClosedBarBrainContext.mq5",
    "tests\TestHistoricalDatasetOrchestrator.mq5"
)

& (Join-Path $PSScriptRoot "sync_session_parity_fix_to_mt5.ps1") `
...
Write-Output "Session parity correction compile passed: 5/5 targets, 0 errors, 0 warnings."
