...
$relativeFiles = @(
    "core\ai\OutcomeFreeM5PathExporter.mqh",
    "tests\TestCurrentFeedJointGeometryM5Exporter.mq5"
)
foreach ($relativeFile in $relativeFiles) {
    ...
    if ($sourceHash -ne (Get-FileHash -LiteralPath $destination -Algorithm SHA256).Hash) {
        throw "IMP-100 outcome-free exporter sync hash mismatch: $relativeFile"
    }
